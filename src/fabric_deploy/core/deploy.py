...
from fabric_cicd import FabricWorkspace, publish_all_items, append_feature_flag, append_feature_flag
...
def run_full(
    *,
    ...
    if dry_run:
        msg = f"🔄 Dry run: Would perform FULL deployment to env={workspace.environment} " f"workspace={workspace.id}"
        logger.info(msg)
